  ...
  return certificates.map((certificate) => {
    return (
      <DocumentsContainer>
        ...
        <button
          className="deletedcertificatebutton"
          onClick={() => handleDelete(certificate.name, certificate._id)}
        >
          <img
            src="https://cdn.onlinewebfonts.com/svg/img_79761.png"
            alt="deleteIcon"
...
function Certificates() {
  ...
  function handleDelete(nameFile, idFile) {
    const deleteFile = storage.ref(
      `Pilots/Pilot-${pilotId}/Certificates/` + nameFile
    );
  ...
  function onSubmit(data) {
    const uploadDocument = storage
      .ref(`Pilots/Pilot-${pilotId}/Certificates/` + name)
      .put(selectedFile);
    uploadDocument.on(
      "state_changed",
      (snap) => {},
      ...
      () => {
        storage
          .ref(`Pilots/Pilot-${pilotId}/Certificates/`)
          .child(name)
          .getDownloadURL()
          .then((url) => {
            axios({
  ...
  return (
    <ComponentContainer>
      ...
      <IconContainer>
        <img
          src="https://cdn.pixabay.com/photo/2017/06/22/02/16/computer-icon-2429310__340.png"
          alt="iconCertificates"
        ></img>
      </IconContainer>
      <p className="description">
        Aca podras incluir todos los certificados que te han permitido conocer,
        aprender o obtener experiencia en algún area especifica. Recuerda que
        solo se muestra el título del documento, ademas los documentos son
        validados y se les asigna un logo especial para darle un valor agregado
        al mostrar tu perfil a los clientes.
      </p>
      <br />
      <button
      ...
      {showadd && (
        <form
          className="certificateformcontainer"
        ...
        >
          ...
          <fieldset className="certificateformfieldset">
            <div className="certificateinput-full-container">
              <label className="certificateformlabel" htmlFor="certificatename">
                Título:{" "}
              </label>
              <input
                id="titlecertificate"
            ...
            <div className="certificateinput-full-container">
              ...
              <input
                ...
                ref={register({
                  required: {
                    value: true,
                    message: "El campo empresa emisora es requerido",
                  },
                })}
              />
            </div>
          ...
          <fieldset className="certificateformfieldset">
            <div className="certificateinput-full-container">
              <label className="certificateformlabel" htmlFor="certificateid">
                ID de la credencial:{" "}
              </label>
              <input
                id="certificateid"
                ...
                ref={register({
                  required: {
                    value: true,
                    message: "El campo credencial es requerido",
                  },
                })}
              />
            </div>
            <div className="error-input-container-certificate">
              <span style={{ color: "red" }}>{errors.credential?.message}</span>
            </div>
          </fieldset>
          <fieldset className="certificateformfieldset">
